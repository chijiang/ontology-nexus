...
class GraphRelationship(Base):
    ...
    properties = Column(JSONB, default=dict)  # 关系的属性（较少使用）
    created_at = Column(DateTime, server_default=func.now())
    ...
    __table_args__ = (
        UniqueConstraint(
            "source_id", "target_id", "relationship_type", name="uq_source_target_rel"
        ),
        ...
        Index(
            "idx_relationships_composite", "source_id", "relationship_type", "target_id"
        ),
    )
...
class SchemaClass(Base):
    ...
    label = Column(JSONB, default=list)  # 人类可读的标签/描述（列表存储多个别名）
...
class SchemaRelationship(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement="auto")
    source_class_id = Column(
        Integer, ForeignKey("schema_classes.id", ondelete="CASCADE"), nullable=False
    )
    target_class_id = Column(
        Integer, ForeignKey("schema_classes.id", ondelete="CASCADE"), nullable=False
    )
